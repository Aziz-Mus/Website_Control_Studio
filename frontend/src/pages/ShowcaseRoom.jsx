...
export default function ShowcaseRoom() {
  ...
  const handleApplyColor = async ({ rgb, brightness: br, sceneId }) => {
    if (!devices.length) { toast.error("No lights"); return; }
    setLoading(true);
    ...
    try {
      if (selectedIds.length > 0 && selectedIds.length < devices.length) {
        ...
        if (res.data.devices) updateStatuses(res.data.devices, "on");
  ...
  const handleActivateAll = async () => {
    ...
    try { const r = await axios.post(`${BASE}/lampu`, { Warna: { Red: 255, Green: 255, Blue: 255 }, Kecerahan: brightness }); if (r.data.devices) updateStatuses(r.data.devices, "on"); } catch { toast.error("Failed"); }
    setLoading(false);
  };
  const handleDeactivateAll = async () => {
    ...
    try {
      ...
      if (r.data.devices) updateStatuses(r.data.devices, "off");
